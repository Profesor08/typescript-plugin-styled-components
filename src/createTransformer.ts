...
function isStyledKeyframesIdentifier(
  name: string,
  { keyframes = ["keyframes"] }: CustomStyledIdentifiers,
) {
  return keyframes.indexOf(name) >= 0;
}

function isStyledCssIdentifier(
  name: string,
  { css = ["css"] }: CustomStyledIdentifiers,
) {
...
function isMinifyableStyledFunction(
  node: ts.Node,
  identifiers: CustomStyledIdentifiers,
) {
  return (
...
function defaultGetDisplayName(
  filename: string,
  ...
  function getIdFromNode(
    ...
    if (
      ...
      const filePath = sourceRoot
        ...
        : fileName;
      return (
        "sc-" + hash(`${getDisplayNameFromNode(node)}${filePath}${position}`)
      );
    }
    return undefined;
  }
      ...
      const visitor: ts.Visitor = (node) => {
        if (
          ...
          if (minifiedTemplate && minifiedTemplate !== node.template) {
            const newNode = context.factory.createTaggedTemplateExpression(
              node.tag,
              node.typeArguments,
              minifiedTemplate,
            );
        ...
        if (
          ...
          if (ssr) {
            const componentId = getIdFromNode(
              node.parent.parent,
              sourceRoot,
